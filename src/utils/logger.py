...
class ScoreLogger:
    ...
    def _save_png(self, input_path, output_path, x_label, y_label, average_of_n_last, show_goal, show_trend, show_legend):
        ...
        plt.plot(x[-average_range:], [np.mean(y[-average_range:])] * len(y[-average_range:]), linestyle="--", label='Average of last {} runs'.format(average_range))
        if show_goal:
            plt.plot(x, [AVERAGE_SCORE_TO_SOLVE] * len(x), linestyle=":", label='Average solving score: {}'.format(AVERAGE_SCORE_TO_SOLVE))
